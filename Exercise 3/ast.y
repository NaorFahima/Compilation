...
%token <ival> INT_NUM IOTA
%token <fval> FLOAT_NUM
%token <op> ADDOP MULOP RELOP XOR
%token <name> ID

%token READ IF ELSE WHILE  FOR INT FLOAT
%token OR AND NOT NAND SWITCH CASE DEFAULT BREAK CONTINUE REPEAT
%type <_type> type
...
%%
program    : declarations |  declarations stmt { 
                     /* if (errors == 0) { for debugging: generate code even if errors found */
					     $2->genStmt (); emit ("halt\n"); 
						 /*} */
				     }
declarations: declarations ID '=' IOTA ';' { if (!(putSymbol ($2,_INT))) 
