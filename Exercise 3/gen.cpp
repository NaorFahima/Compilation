...
{	
	...
	if(left_operand_result._type != VARIABLE && right_operand_result._type != VARIABLE){
		...
		if(left_operand_result._type == INT_NUMBER){
			left = atoi(left_operand_result._string);
			right = atoi(right_operand_result._string);
			result._type = INT_NUMBER;
		} 
		else {				
			left = atof(left_operand_result._string);
			right = atof(right_operand_result._string);
			result._type = FLOAT_NUMBER;
		}
		switch (_op)
		{ 
		case PLUS:
		...
		if(left_operand_result._type == INT_NUMBER)
			sprintf(result._string, "%d", (int) exp);
		else 
			sprintf(result._string, "%.2f", exp);
...
{ 
	Case *myCase = _caselist;
	int finalLabel,defaultLabel,result;
	if(_exp->_type == _INT){
		_exp->genExp();
		emit("\n");
		while(myCase != NULL){
			myCase->_label = newlabel();
			emit("if _t1 == %d goto label%d\n",myCase->_number,myCase->_label);
			myCase = myCase->_next;
		}
		...
		myCase = _caselist;
		while(myCase != NULL){
			emitlabel(myCase->_label);
			myCase->_stmt->genStmt();
			emit("goto label%d\n",finalLabel);
			myCase = myCase->_next;
		}
		...
		emit("\n");
		emitlabel(finalLabel);
	}
	else
		errorMsg("The numeber in the Switch is not INT.\n");
...
{	
	...
	if(_exp->_type == _INT){
		...
		errorMsg("The numeber in the Repeat is not INT .\n"); //or not positive INT
